...
const wordLen = 32 << (^uint(0) >> 63) // either 32 or 64
...
func (s *IntSet) Copy() *IntSet {
	var ns = IntSet{make([]uint, len(s.words))}
	copy(ns.words, s.words)
	return &ns
}

// Elems return a slice of all elements
func (s *IntSet) Elems() []uint {
	var elems = []uint{}
	for word := range s.words {
		for bit := uint(0); bit < wordLen; bit++ {
			if (1<<bit)&s.words[word] != 0 {
				elems = append(elems, uint(word)*wordLen+bit)
			}
		}
	}
	return elems
...
func main() {
	var s IntSet
	s.AddAll(1, 3, 199, 200000, 765)
	fmt.Println("Set s contains:")
	for _, ele := range s.Elems() {
		fmt.Println("\t", ele)
	}
}
